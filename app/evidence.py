...
class EvidenceEntry:
    ...
    lay_summary: str = ""        # 1-2 sentence plain-English rewrite (added post-synthesis for the hover popup)
...
class EvidenceLedger:
    ...
    def set_lay_summary(self, label: str, lay_summary: str) -> None:
        """Attach a plain-English summary (generated post-synthesis) to a cited entry."""
        with self._lock:
            for entry in self._by_key.values():
                if entry.label == label:
                    entry.lay_summary = (lay_summary or "").strip()
                    return
